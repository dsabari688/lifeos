...
export default function App() {
  ...
  const handleClearNotifications = async () => {
    try {
      const res = await fetch("/api/notifications/clear-unread", { method: "POST" });
      if (res.ok) {
        await hydrateSystemData();
      }
    } catch (err) {
  ...
  if (!isLoggedIn) {
    return (
      <div className="fixed inset-0 z-50 bg-slate-50 flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-white rounded-2xl border border-slate-100 shadow-xl p-8 space-y-6 animate-in fade-in zoom-in-95 duration-200">
          
          <div className="text-center space-y-2">
            <div className="w-14 h-14 rounded-full bg-amber-50 mx-auto flex items-center justify-center text-amber-500">
              <Terminal className="w-7 h-7" />
            </div>
            <h2 className="font-display font-black text-2xl text-slate-800 leading-tight">Secure Terminal Port</h2>
            <p className="text-xs text-slate-500 font-sans">Alex, unlock your daily LifeOS parameters interface.</p>
          </div>

          <div className="space-y-4 font-sans text-sm">
            <div className="space-y-1.5">
              <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide">Client Node Name</label>
              <input
                type="text"
                required
                value={loginUsername}
                onChange={(e) => setLoginUsername(e.target.value)}
                className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-amber-500"
              />
            </div>

            <div className="space-y-1.5">
              <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide">Relay Email Identity</label>
              <input
                type="email"
                required
                value={loginEmail}
            ...
            <div className="p-3 bg-amber-50/50 rounded-xl border border-amber-100 flex gap-2 items-start mt-4">
              <Info className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
              <p className="text-[11px] text-amber-800 font-sans leading-relaxed">
                Using local Sandboxed database layers. System credentials will save directly in memory for active sessions.
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full py-3 bg-amber-500 hover:bg-amber-600 text-slate-950 font-display font-bold text-sm rounded-xl shadow-md transition-all active:scale-[0.98] cursor-pointer flex items-center justify-center gap-2"
          >
            Authenticate Node
        ...
        <div className="p-3 border-t border-white/5 space-y-2">
          {isSidebarOpen && (
            <div className="flex items-center gap-2.5 px-2 bg-white/5 py-2.5 rounded-xl border border-white/5">
              <img
                src={osData.profile.avatar}
                alt={osData.profile.name}
                className="w-9 h-9 rounded-full object-cover border border-amber-500"
              />
              <div className="min-w-0">
                <span className="text-xs font-bold text-slate-200 block truncate">{osData.profile.name}</span>
