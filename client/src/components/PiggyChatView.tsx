...
export const PiggyChatView: React.FC<PiggyChatViewProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setPiggyState("SPEAKING");
    
    // Reset state back to IDLE after a short verbal feedback animation delay
    setTimeout(() => {
      setPiggyState("IDLE");
    }, 2500);
  };
  ...
  const startVoiceCaptureMode = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(async () => {
        ...
        setPiggyState("SPEAKING");
        setTimeout(() => setPiggyState("IDLE"), 3000);
      }, 1500);
    }, 4000);
  };
  ...
  const requestMicAndStart = async () => {
    try {
      // 1. Prompt browser mic permissions dialog
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
      setMicGranted(true);
      // Immediately stop track to unlock hardware mic LED lights
      stream.getTracks().forEach(track => track.stop());
      
      // 2. Locate native Web Speech listeners
      ...
      if (SpeechRecognition) {
        ...
        recognition.onresult = async (event: any) => {
          ...
          setPiggyState("SPEAKING");
          setTimeout(() => setPiggyState("IDLE"), 2500);
        };
      ...
      setMicGranted(false);
      // Give feedback of fallback simulation
      startVoiceCaptureMode();
    }
  };
