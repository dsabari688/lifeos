            ...
            >
              <div>
                <div className="flex items-start justify-between">
                  <span className="text-3xl filter drop-shadow hover:scale-110 transition-transform cursor-pointer select-none">
                    {iconEmoji}
                  </span>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-xs p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-50 pb-4 mb-4">
          ...
          <div className="flex items-center gap-4 font-mono text-[9px] font-bold text-slate-400">
            ...
            <div className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded bg-amber-500 animate-pulse" /> Partial
            </div>
            <div className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded bg-slate-100" /> Overdue
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-7 gap-2">
          {GRID_DAYS.map((day) => {
            // Days offset logic
            // June 21, 2026 is Day 21
            // Generate some coloring logic to make it look incredibly realistic!
            const isToday = day === 21;
            const isFuture = day > 21;
            
            let bgClass = "bg-slate-100";
            if (isFuture) {
              bgClass = "bg-slate-50/50 text-slate-300/60";
            } else if (day % 4 === 0) {
              bgClass = "bg-emerald-500 text-white";
            } else if (day % 7 === 0) {
              bgClass = "bg-slate-100 text-slate-400"; // Missed
            } else if (day % 3 === 0) {
            ...
            return (
              ...
              >
                <span>{day}</span>
                {isToday && (
                  <span className="absolute bottom-1 w-1 h-1 rounded-full bg-amber-950" />
                )}
              </div>
            );
          })}
